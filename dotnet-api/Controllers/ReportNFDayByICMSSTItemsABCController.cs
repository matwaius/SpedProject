...
namespace dotnet_api.Controllers
{
    ...
    [ApiController]
    public class ReportNFDayByICMSSTItemsABCController : ControllerBase
    {
        private readonly IFilesRepository _repository;
        private readonly IMapper _mapper;
        private IConfiguration _configuration { get; }
        public ReportNFDayByICMSSTItemsABCController(IFilesRepository repository, IMapper mapper, IConfiguration configuration)
        {
            ...
            try
            {
                ...
                foreach (string line in file.Split('\n'))
                {
                    if (line != "")
                    {
                        ...
                        {
                            if (Library.GetDecimal(data[18]) > 0)//VALOR ICMS ST
                            {
                                C170 c170 = new C170();
                ...
                {
                    decimal Acumulado = 0;
                    foreach (C170 c170 in list.OrderByDescending(o => o.VL_ICMS_ST))
                    {
                        c170.PERC = (c170.VL_ICMS_ST * 100) / total;
                        Acumulado += c170.PERC;
