...
export default {
  ...
  methods: {
    ...
    FilterBKP () {/*OK - Sem o moment para corrigir a data que está vindo datetime do backend */
      axios
        .post('https://localhost:7258/api/ReportNFDayByICMSST?dateStart=' + this.date + '&dateEnd=' + this.date2).then(response => {
      ...
      axios
        .post('https://localhost:7258/api/ReportNFDayByICMSST?dateStart=' + this.date + '&dateEnd=' + this.date2).then(response => {
          ...
          this.list.forEach(d => {
            ...
            this.list2.push({ DT_DOC, VL_ICMS_ST })
          })
          this.loaded = true
          console.log(response.data)
          console.log('teste')
