<template>
  <div>
    <Dashboard
        title="Relatório de Nota Fiscal por ICMS ST"
    ...
    methods: { 
      ...
      async getItems(e){
        if(this.loading == false){
            ...
            this.total_itens=0;
              try{
                await api.post('/ReportNFDayByICMSSTItems?date=' + validation.parseDate(e)+'&indOperacao='+this.filtros[0].ind)
                    .then(response => {
                        for (let c = 0; c < response.data.length; c++) {
                            this.tableitems.push(response.data[c]);
                            this.total_itens = Math.round(this.total_itens* 100) / 100  + Math.round((response.data[c].VL_ICMS_ST)* 100) / 100 ; 
                        }
      ...
      async getItemsABC(e){
        if(this.loading == false){
            ...
            this.total_itens=0;
              try{
                await api.post('/ReportNFDayByICMSSTItemsABC?dateStart=' + this.filtros[0].dataInicial + '&dateEnd=' + this.filtros[0].dataFinal +'&indOperacao='+this.filtros[0].ind +'&A=' +this.filtros[0].curvaA +'&B=' + this.filtros[0].curvaB +'&C=' + this.filtros[0].curvaC + '&CurvaSel=' + e)
                    .then(response => {
                        for (let c = 0; c < response.data.length; c++) {
                            this.tableitems.push(response.data[c]);
                        }
