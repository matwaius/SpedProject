    ...
    methods: { 
      ...
      async getItems(e){
        this.tableheader=[
              ...
              { text: "Valor", value: "VL_ITEM",align:"end", width: 150, divider:false,  sortable: true },
          ];
        this.tableitems=[];
        this.total_itens=0;
        await api.post('/ReportNFDayByUFItems?dateStart=' + this.filtros[0].dataInicial + '&dateEnd=' + this.filtros[0].dataFinal+'&UF=' + e+'&indOperacao='+this.filtros[0].ind)
            .then(response => {
                for (let c = 0; c < response.data.length; c++) {
                    this.tableitems.push(response.data[c]);
                    this.total_itens = Math.round(this.total_itens* 100) / 100  + Math.round((response.data[c].VL_ITEM - response.data[c].VL_DESC)* 100) / 100 ; 
                }
