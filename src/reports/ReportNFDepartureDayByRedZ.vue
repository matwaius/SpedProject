<template>
  <div>
    <h1>Relatório - Relatório de Redução Z</h1>
  ...
  methods: {
    Filter () {
      /*reports.ReportCFDay('2021-06-01', '2021-06-30').then(response => {
        ...
        .catch(error => console.log(error))*/
      axios
        .post('https://localhost:7258/api/ReportNFDepartureDayByRedZ?dateStart=' + this.dateStart + '&dateEnd=' + this.dateEnd).then(response => {
          this.list = response.data
          console.log(response.data) })
    }
  },
