<template>
  <div>
    <Dashboard
        title="Relatório de Nota Fiscal Quantidade por UF"
  ...
  export default {
    ...
    methods: { 
      async getRel () {
          this.limpaDados();
          await api.post('/ReportNFQtdDayByUF?dateStart=' + this.filtros[0].dataInicial + '&dateEnd=' + this.filtros[0].dataFinal+'&indOperacao='+this.filtros[0].ind)
            .then(response => {
      ...
      async getNotas(e){
        ...
        this.tableitems=[];
        this.total_itens=0;
        await api.post('/ReportNFQtdDayItemsByUF?dateStart=' + this.filtros[0].dataInicial + '&dateEnd=' + this.filtros[0].dataFinal+'&UF=' + e+'&indOperacao='+this.filtros[0].ind)
            .then(response => {
              for (let i = 0; i < response.data.length; i++) {
